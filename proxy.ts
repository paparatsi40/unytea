...
function routeNeedsAuth(pathname: string): boolean {
  ...
  const hasLocale =
    segments[0] !== undefined && (locales as readonly string[]).includes(segments[0]);
  const path = hasLocale ? "/" + segments.slice(1).join("/") : pathname;

  return (
    path.startsWith("/auth") || path.startsWith("/dashboard") || path.startsWith("/onboarding")
  );
}
...
const authMiddleware = auth((req) => {
  ...
  // For /auth/* (no locale prefix), prefer the locale of the page the visitor
  // came from (Referer) over next-intl's Accept-Language fallback. Otherwise a
  // visitor who was on /es/* and clicks "Sign in" lands on /en/auth/signin
  // because their browser's Accept-Language is English. Bookmarks and external
  // links still fall through to the default detection below.
  if (pathname.startsWith("/auth")) {
    const referer = req.headers.get("referer");
    if (referer) {
      try {
        const refererFirstSeg = new URL(referer).pathname.split("/").filter(Boolean)[0];
        if (refererFirstSeg && (locales as readonly string[]).includes(refererFirstSeg)) {
          return NextResponse.redirect(
            new URL(`/${refererFirstSeg}${pathname}${req.nextUrl.search}`, req.url)
          );
        }
      } catch {
  ...
  const isGoingToOnboarding =
    req.nextUrl.searchParams.has("newUser") || req.headers.get("referer")?.includes("/onboarding");
...
export const config = {
  matcher: [
    ...
    // /sitemap.xml -> /en/sitemap.xml -> 404. Lo mismo para /og (la dynamic OG image route
    // vive en app/og/, no bajo [locale]).
    "/((?!api|_next/static|_next/image|favicon.ico|manifest\\.json|sw\\.js|sitemap\\.xml|robots\\.txt|og$|icons/|excalidraw-assets/|.*\\.(?:svg|png|jpg|jpeg|gif|webp|woff2?|xml|txt)$).*)",
  ],
};
