      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="animate-pulse rounded-xl border border-gray-200 bg-white p-4 md:p-6"
            >
              <div className="flex items-center space-x-3 md:space-x-4">
                <div className="h-14 w-14 rounded-full bg-gray-200 md:h-16 md:w-16" />
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3">
          {filteredMembers.map((member) => (
            <MemberCard key={member.id} member={{ ...member, communityId }} />
          ))}
        </div>
      )}
    </div>
