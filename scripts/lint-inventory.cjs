...
for (const [rule, items] of sorted) {
  const samples = Array.from(new Set(items.map(function (i) {
...
const riskyRules = ['react-hooks/exhaustive-deps', 'react-hooks/rules-of-hooks', '@typescript-eslint/no-explicit-any', '@typescript-eslint/ban-ts-comment'];
for (const [rule, items] of sorted) {
  if (riskyRules.includes(rule) || rule.toLowerCase().includes('react-hooks') || rule.toLowerCase().includes('react-compiler')) {
