...
export function getUpgradeMessage(
  ...
  if (currentPlan === "FREE") {
    return `Upgrade to Professional ($129/month) to unlock ${feature}`;
  }
...
export function getUpgradeRecommendation(
  ...
  for (let i = currentIndex + 1; i < plans.length; i++) {
    ...
    const currentCost = calculateMonthlyCost(currentPlan, usage);
    const nextCost = calculateMonthlyCost(nextPlan, usage);
